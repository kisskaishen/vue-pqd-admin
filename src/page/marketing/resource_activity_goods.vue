    ...
    <div class="item">
      ...
      <div class="content">
      	<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
			...
			<div class="warp_form" v-show="action=='down'">
			  <el-form-item label="砍价初始价">
			  	<div class="width_200">
			    	<el-input :disabled="btn=='detail'" :maxlength="5" size="small" v-model.trim="ruleForm.initial_price" style="width:160px;"></el-input><span style="padding-left:10px;"> 元</span>
			  	</div>
			  </el-form-item>
		  </div>
		  <div class="warp_form_kan" v-show="action=='down'">
		  	  <div class="leftLabel">砍价次数</div>
		  	  <div class="rightLabel">
		  	  	<div class="input_start">
		  	  		<input :disabled="btn=='detail'" v-model="ruleForm.min_cut_times" :class="{'input_check':true, 'disabled_class':btn=='detail'}" :maxlength="5" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')">
		  	  	</div>
		  	  	<div class="input_center">到</div>
		  	  	<div class="input_end">
		  	  		<input :disabled="btn=='detail'" v-model="ruleForm.max_cut_times" :class="{'input_check':true, 'disabled_class':btn=='detail'}" :maxlength="5" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')">
		  	  	</div>
		  	  </div>
		  	  <div class="explain">
		  	  	(注：根据填写的砍价次数，系统会随机设置每次砍价的金额，直至最后价格为0)
		  	  </div>
		  </div>

		  <div class="warp_form">
			  <el-form-item label="操作内容">
			      <el-radio :disabled="btn=='detail'" v-model="ruleForm.audit_status" label="1">通过</el-radio>
  				  <el-radio :disabled="btn=='detail'" v-model="ruleForm.audit_status" label="2">驳回</el-radio>
			  </el-form-item>
		  </div>
		  <div class="warp_form">
			  <el-form-item label="操作说明" id="desc" prop="desc">
			  	<div class="width_600">
			    	<el-input :disabled="btn=='detail'" size="small" type="textarea" v-model="ruleForm.desc"></el-input>
			  	</div>
			  </el-form-item>
		  </div>
		  <el-form-item>
		  	<div class="submit" v-show="show_btn">
		    	<el-button type="primary" @click="submitForm('ruleForm')">提交</el-button>
		  	</div>
		  </el-form-item>
       </el-form>
       	<div class="operation">
       		<div class="label_left">操作记录：</div>
       		<div class="label_right">
				  <el-table
				    :data="goodsDetailObj.audit_log"
				    border
				    style="width: 100%">
				    <el-table-column
				      prop="audit_status"
				      ...
				      <template slot-scope="scope">
				      		...
				      		<span v-if="scope.row.audit_status=='2'">审核不通过</span>
				      </template>
				    </el-table-column>
				    <el-table-column
				      prop="desc"
 ...
 export default {
   data () {
      ...
      var validateDesc = (rule, value, callback) => {
        if (value === '') {
        	if(this.ruleForm.audit_status!='1'){
          		callback(new Error('请输入拒绝备注'));
        	}else{
        		callback();
        	}
        }else{
     ...
     return {
		...
		content_title:{
			title1:"商品列表",
			title2:"商品详情",
			path1:'/home/marketing/resource_activity',
			path2:'/home/marketing/resource_activity_detail',
		},
		ruleForm:{
			name:'',
			limit_reach:'',
...
//          { required: true, message: '请填写操作说明', trigger: 'blur' }
    ...
    created(){
		...
		if(this.title=='resource_activity_detail'){
			this.content_title.path2 = "/home/marketing/resource_activity_detail?activity_id="+this.activity_id+"&action="+this.action;
		}
		var data = {
		...
		Market.auditGoodsInfo('Activities/auditGoodsInfo',data,this,this.btn);
    },
   components: {
		breadcrumb
   },
   methods:{
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
          	if(this.action == "down"){
							if(this.ruleForm.initial_price<=0||this.ruleForm.initial_price==undefined){
									this.$message({
										type:"info",
										message:"初始价格不对"
									})
									return
							}else if(this.ruleForm.min_cut_times<=0||this.ruleForm.min_cut_times==undefined){
										this.$message({
											type:"info",
											message:"砍价最小值错误"
										})
										return
							}else if(this.ruleForm.max_cut_times<=0||this.ruleForm.max_cut_times==undefined){
										this.$message({
											type:"info",
											message:"砍价最大值错误"
										})
										return
							}else if(this.ruleForm.min_cut_times-this.ruleForm.min_cut_times>0){
										this.$message({
											type:"info",
											message:"砍价最小值不能高于砍价最大值"
										})
										return
							}
	          	var params = {
	          		initial_price:this.ruleForm.initial_price,
	          		min_cut_times:this.ruleForm.min_cut_times,
	          		max_cut_times:this.ruleForm.max_cut_times,
	          	}
	          	var data = {
	          		id:this.id,
          	...
          	Market.auditGoods('Activities/auditGoods',data,this,"down");
          } else {
