...
export default {
    	...
    	api.commonAjax({
            ...
            success:function(result){
            	if(result.code==200){
	                ...
	                that.reg_typeOption = jsonData.reg_type;
	                that.reg_typeOption.unshift({label:'全部',value:''})
               }
            }
        })
    },
    	...
    	api.commonAjax({
            ...
            success:function(result){
            	if(result.code==200){
					...
					that.send_time_start = that.form.send_time.start;
					that.send_time_end = that.form.send_time.end;

					that.use_time_start = that.form.use_time.start;
					that.use_time_end = that.form.use_time.end;
    ...
    handleCoupon(url,data,that,title){
    	api.commonAjax({
            method:"post",
            url:url,
            data:data,
            success:function(result){
                ...
                if(title=="quduoduo"){
              		that.$router.push({path:'/home/quduoduo/coupon_manager_quduoduo'})
                }else{
              		that.$router.push({path:'/home/couponManager/couponList'})
                }
            		that.$message({
            			type:"success",
            			message:result.msg
