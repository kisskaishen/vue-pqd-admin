    ...
    <div class="tits">
      <div class="arrangement-content">
        <ul class="arrangement-pictures">
          ...
          <li v-show="dataImageArr!=''" class="up-load-img"><img :src="dataImageArr" class="avatar"><i class="delete" @click="deleteBig(type)"></i></li>
        </ul>
...
import routers from '../router/index.js';
 export default {
   ...
   props:["type","goodsId","upLoadData","keys","imageArr",,"explain"],
   components: {
   ...
   watch:{
		imageArr(newVal,oldVal){
			...
			if(this.type=="banner99"||this.type=="img99"){
				if(this.imageArr==''||this.imageArr==undefined){
			...
			}else if(this.type=="category2"||this.type=="category"){
				if(this.imageArr==''||this.imageArr==undefined){
		            this.strCount = true;
		         }else{
		            this.isShow = true;
     ...
     deleteBig(type,index){
      ...
      successUpLoad(response, file, fileList){
          if(response.code == 200){
              ...
              if(this.type=="banner99"||this.type=="img99"||this.type=="category"){
                 this.isShow = true;
                 this.dataImageArr = response.data;
                 this.strCount = false;
                 this.$emit("addImg",[this.dataImageArr,this.type])
               }else if(this.type=="category2"){
                 this.isShow = true;
                 this.dataImageArr = response.data;
                 this.strCount = false;
                 this.$emit("addImg",[this.dataImageArr,this.type])
