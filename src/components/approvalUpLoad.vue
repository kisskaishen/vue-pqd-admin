...
import routers from '../router/index.js';
 export default {
   ...
   props:["type","goodsId","upLoadData","keys","imageArr","type"],
   components: {
   ...
   watch:{
		imageArr(newVal,oldVal){
			this.dataImageArr = this.imageArr;
			if(this.type=="originalImg"||this.type=="listImg"||this.type=="goodSkuImg"){
				if(this.imageArr==''||this.imageArr==undefined){
      ...
      successUpLoad(response, file, fileList){
          if(response.code == 200){
              this.$message.warning(`${ response.msg }`);
               ...
               }else if(this.type=='customer'||this.type=='originalImg'||this.type=='listImg'||this.type=="goodSkuImg"){
                 this.isShow = true;
                 this.dataImageArr = response.data;
                 this.strCount = false;
                 this.$emit("deleteBig",[this.dataImageArr,this.type,'add',this.keys])
               }
          }else{
